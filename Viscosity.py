...
def Viscosity_r(r_0, r_1, r_2, v_0, v_1, v_2, a_0, a_1, a_2, SS, RO):
    rc = (r_0 + r_1 + r_2) / 3.0
    # lc = sqrt(square(xc) + square(yc))
    ac = (a_0 + a_1 + a_2) / 3.0
    abs_acs = (ac ** 2).sum(axis=1)
    abs_acs = where(abs_acs < 1.0e-7, abs_acs + 1, abs_acs)
    # abs_acs = abs_acs + ones(abs_acs.size) * (abs_acs < 1.e-7)
    n_ac = ac / abs_acs[:, np.newaxis]
    '''cosThetta = axc / abs_acs
    sinThetta = ayc / abs_acs'''
    DVXDX = -2 * Area((v_1[:, 0] + v_2[:, 0]), (v_0[:, 0] + v_2[:, 0]), (v_1[:, 0] + v_0[:, 0]), r_0[:, 1], r_1[:, 1],
                      r_2[:, 1])
    DVYDY = 2 * Area((v_1[:, 1] + v_2[:, 1]), (v_0[:, 1] + v_2[:, 1]), (v_1[:, 1] + v_0[:, 1]), r_0[:, 0], r_1[:, 0],
                     r_2[:, 0])
    DVXDY = 2 * Area((v_1[:, 0] + v_2[:, 0]), (v_0[:, 0] + v_2[:, 0]), (v_1[:, 0] + v_0[:, 0]), r_0[:, 0], r_1[:, 0],
                     r_2[:, 0])
    DVYDX = -2 * Area((v_1[:, 1] + v_2[:, 1]), (v_0[:, 1] + v_2[:, 1]), (v_1[:, 1] + v_0[:, 1]), r_0[:, 1], r_1[:, 1],
                      r_2[:, 1])
